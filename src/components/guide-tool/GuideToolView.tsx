        ...
        <NitroCardView uniqueKey="guide-tool" className="illumina-guide-tool w-[280px]" windowPosition={ DraggableWindowPosition.TOP_LEFT }>
            ...
            <NitroCardContentView className={`text-black ${[ GuideSessionState.GUIDE_ONGOING, GuideSessionState.USER_ONGOING ].includes(sessionState) || (sessionState === GuideSessionState.USER_FEEDBACK) || (sessionState === GuideSessionState.GUIDE_ACCEPT) ? "!px-px" : ""}`}>
                ...
                { (sessionState === GuideSessionState.USER_FEEDBACK) &&
                    <GuideToolUserFeedbackView userName={ ongoingUsername } userFigure={ ongoingFigure } /> }
                { (sessionState === GuideSessionState.USER_THANKS) &&
                    <GuideToolUserThanksView onCloseClick={ event => processAction("close") } /> }
                { (sessionState === GuideSessionState.USER_NO_HELPERS) &&
                    <GuideToolUserNoHelpersView onCloseClick={ event => processAction("close") } /> }
                { (sessionState === GuideSessionState.USER_SOMETHING_WRONG) &&
                    <GuideToolUserSomethingWrogView onCloseClick={ event => processAction("close") } /> }
