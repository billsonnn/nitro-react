...
export const SongPlaylistView: FC<SongPlaylistViewProps> = props =>
{
    ...
    return (<>
        <div className="bg-primary container mx-auto flex max-w-full justify-center rounded py-3 sm:px-4">
        ...
        <div className="h-100 overflow-y-scroll py-2">
            <Flex column gap={ 2 }>
                ...
                {
                    return <Flex gap={ 1 } key={ index } className={ "cursor-pointer " + (selectedItem === index ? "border border-muted border-2 rounded" : "border-2") } alignItems="center" onClick={ () => setSelectedItem(prev => prev === index ? -1 : index) }>
        ...
        <><div className="playlist-bottom ms-5 p-1">
        ...
        { (playlist && playlist.length > 0) &&
            <>
                ...
                <Button variant="success" onClick={ () => playPause(furniId, selectedItem) }>
