        ...
        <ContextMenuView objectId={ avatarInfo.roomIndex } category={ RoomObjectCategory.UNIT } userType={ RoomObjectType.PET } onClose={ onClose } collapsable={ true }>
            ...
            { (mode === MODE_SADDLED_UP) &&
                <>
                    ...
                    <ContextMenuListItemView onClick={ event => processAction("toggle_riding_permission") }>
                        ...
                        <input className="illumina-input" type="checkbox" checked={ !!avatarInfo.publiclyRideable } readOnly={ true } />
                    </ContextMenuListItemView>
                    { (petRespectRemaining > 0) &&
                        <ContextMenuListItemView onClick={ event => processAction("respect") }>
                            { LocalizeText("infostand.button.petrespect", [ "count" ], [ petRespectRemaining.toString() ]) }
            ...
            { (mode === MODE_MONSTER_PLANT) &&
                <>
                    ...
                    { roomSession.isRoomOwner && avatarInfo.dead &&
                    ...
                    { !avatarInfo.dead && (avatarInfo.level === avatarInfo.maximumLevel) && avatarInfo.breedable &&
                        <>
                            <ContextMenuListItemView disabled onClick={ event => processAction("toggle_breeding_permission") }>
                                { LocalizeText("infostand.button.toggle_breeding_permission") }
                                <input className="illumina-input" type="checkbox" checked={ avatarInfo.publiclyBreedable } />
                            </ContextMenuListItemView>
                            <ContextMenuListItemView disabled onClick={ event => processAction("breed") }>
                                { LocalizeText("infostand.button.breed") }
                            </ContextMenuListItemView>
