...
{
    const { objectId = -1, category = -1, userType = -1, fades = false, onClose = null, classNames = [], className = "", style = {}, children = null, collapsable = false, ...rest } = props
    ...
    const updatePosition = useCallback((bounds: NitroRectangle, location: NitroPoint) =>
    {
        ...
        {
            offset = (offset + ((bounds.height > 50) ? 20 : 0))
        }
        else
        {
            offset = (offset - 14)
    ...
    {
        const newClassNames: string[] = [ "illumina-card absolute flex flex-col px-2 py-3 min-w-[157px] z-50" ]
    ...
    return <div ref={ elementRef } className={ getClassName } style={ getStyle } { ...rest }>
