...
const useNotificationState = () =>
{
    ...
    useMessageEvent<ActivityPointNotificationMessageEvent>(ActivityPointNotificationMessageEvent, event =>
    {
        ...
        const imageUrl = GetConfiguration<string>("image.library.notifications.url", "").replace("%image%", parser.type.toString())
    ...
    const onRoomEnterEvent = useCallback(() =>
    {
        ...
        else
        {
            ...
            const imageUrl = GetConfiguration<string>("image.library.notifications.url", "").replace("%image%", "chatdisclaimer")

            showSingleBubble(LocalizeText("mod.chatdisclaimer"), NotificationBubbleType.INFO, imageUrl)
    ...
    return { alerts, bubbleAlerts, confirms, simpleAlert, showNitroAlert, showTradeAlert, showPurchaseAlert, showConfirm, showSingleBubble, closeAlert, closeBubbleAlert, closeConfirm }
}
